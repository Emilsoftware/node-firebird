...
SQLVarText.prototype.decode = function (data, lowerV13) {
    ...
    if (this.subType > 1) {
        ...
        ret = data.readText(this.length, DEFAULT_ENCODING)?.trim();
    } else if (this.subType === 0) {
        // without charset definition Maury Mattia Simo Ago 2023
        let tmpPos = data.pos;
        data.readText(this.length, DEFAULT_ENCODING)?.trim();
        for (let i = tmpPos; i < tmpPos + this.length; i++) {
            ret += String.fromCharCode(data.buffer[i]);
        }
        ret = ret.trim();
    } else {
...
function getOffset(date = new Date()) {
    // Definisci le date di inizio e fine dell'ora legale per il 2024
    const daylightSavingStart = new Date('2024-03-31T02:00:00'); // Ora legale inizia il 31 marzo 2024 alle 2:00
    const daylightSavingEnd = new Date('2024-10-27T03:00:00');   // Ora solare inizia il 27 ottobre 2024 alle 3:00
    
    // Controlla se la data è compresa tra le date di inizio e fine dell'ora legale
    if (date >= daylightSavingStart && date < daylightSavingEnd) {
      var offset = 60; // GMT+2 (Ora legale)
    } else {
      var offset = 1;  // GMT+1 (Ora solare)
    }
...
SQLVarDate.prototype.decode = function (data, lowerV13) {
    ...
    if (!lowerV13 || !data.readInt()) {
        ...
        d.setMilliseconds((ret - DateOffset) * TimeCoeff + offset * MsPerMinute);
        return d;
    }
...
SQLVarTime.prototype.decode = function (data, lowerV13) {
    ...
    if (!lowerV13 || !data.readInt()) {
        ...
        d.setMilliseconds(Math.floor(ret / 10) + offset * MsPerMinute);
        return d;
    }
    return null;
};
...
SQLVarTimeStamp.prototype.decode = function (data, lowerV13) {
    var date = data.readInt(); // Giorni dal 17/11/1858
    var time = data.readUInt(); // Fractions of a day (1/86400 sec)

    var offset = getOffset() * MsPerMinute; // Converti l'offset in millisecondi
    var d = new Date(0); // Epoch

    if (!lowerV13 || !data.readInt()) {
        // Calcola i millisecondi del timestamp
        var milliseconds = (date - DateOffset) * TimeCoeff + Math.floor(time / 10);

        d.setMilliseconds(milliseconds + offset); // Applica l'offset direttamente ai millisecondi

        /**
        console.log("date:", date);
        console.log("time:", time);
        console.log("offset (ms):", offset);
        console.log("computed milliseconds:", milliseconds);
        console.log("final date:", d.toISOString());
         */
                    ...
                    const read = () => {
                        statement.connection.getSegment(blob, (err, ret) => {
                            ...
                            if (ret.buffer) {
                                ...
                                infoValue.value += data.toString('latin1');
                            }
